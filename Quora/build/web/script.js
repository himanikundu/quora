...
function validateEmail(event) {
    var emailForm = event.currentTarget,
        ...
        emailCheck = new RegExp("[A-Za-z0-9]+@[A-Za-z]+[.][A-Za-z]+", "i"),
...
function validatePassword(event) {
    var passwordForm = event.currentTarget,
        ...
        passCheck = new RegExp("([A-Za-z0-9^\s]){8,}", "i"),
...
function validateMobile(event) {
    var mobileForm = event.currentTarget,
        mobile = mobileForm.value,

        mobileCheck = new RegExp("[0]{1}[0-9]{9}", "i"),
        validMobile = mobileCheck.test(mobile);

        mobileError = document.getElementById("mobile_err");

    mobileError.innerHTML = "";

    if(!validMobile) {
        mobileError.innerHTML = "Invalid mobile number<br />";
    }
}
...
function validateUsername(event) {
    var usernameForm = event.currentTarget,
        ...
        usernameCheck = new RegExp("([A-Za-z0-9^\s]){8,}", "i"),
